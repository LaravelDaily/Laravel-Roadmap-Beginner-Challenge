        ...
        <div class="row justify-content-center">
            <div class="col-md-8">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="{{ route('backend.articles.update', $article) }}" method="POST"
                            ...
                            <div class="form-group">
                                <textarea name="excerpt" class="form-control @error('excerpt') is-invalid @enderror"
                                          rows="2" placeholder="Excerpt">{{ old('excerpt', $article->excerpt) }}</textarea>
                                @error('excerpt')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
                            ...
                            <div class="form-group">
                                <textarea name="body" class="form-control @error('body') is-invalid @enderror" rows="5"
                                          placeholder="Body">{{ old('body', $article->body) }}</textarea>
                                @error('body')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
                            ...
                            <div class="form-group">
                                <select class="form-control @error('category_id') is-invalid @enderror"
                                    ...
                                    @forelse($categories as $category)
                                        <option
                                            value="{{ $category->id }}" {{ $category->id == old('category_id',  $article->category_id) ? 'selected' : '' }}>{{ $category->name }}</option>
                                    @empty
                            ...
                            <div class="form-group">
                                <select multiple class="form-control @error('tag_id') is-invalid @enderror"
                                    ...
                                    @forelse($tags as $tag)
                                        <option
                                            value="{{ $tag->id }}" {{ in_array($tag->id, old('tag_id',  $article->tags->pluck('id')->toArray())) ? 'selected' : '' }}>{{ $tag->name }}</option>
                                    @empty
