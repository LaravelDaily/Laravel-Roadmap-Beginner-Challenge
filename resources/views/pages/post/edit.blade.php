                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="post" action="{{ route('post.update', $post->id) }}" enctype="multipart/form-data">
                        ...
                        <select name="tag_ids[]" id="tag_ids" multiple class="block mt-1 w-full">
                            @foreach($tags as $tag)
                                <option {{ $post->tags->contains($tag->id) ? 'selected' : '' }} value="{{$tag->id}}"> {{$tag->name}}</option>
                            @endforeach
                        </select>
                        ...
                        <textarea name="description" class="my-2 w-full border-gray-300 rounded px-3 py-2 outline-none" cols="30" rows="5"> {{ $post->description }}</textarea>
