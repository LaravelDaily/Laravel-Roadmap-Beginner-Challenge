    ...
    <section class="bg-white dark:bg-gray-900">
        <div class="py-8 px-4 mx-auto max-w-screen-xl lg:py-16 lg:px-6">
            <div class="mx-auto max-w-screen-xl text-center lg:mb-16 mb-8">
                <h2 class="mb-4 text-3xl lg:text-4xl text-center tracking-tight font-extrabold text-gray-900 dark:text-white">{{$articles[0]->category->name}}</h2>
                @foreach($categories as $id => $name)
                    <span
                        class="bg-indigo-100 @if($articles[0]->category->id == $id) border-b-2 border-indigo-600 @endif text-indigo-800 text-md m-1 font-medium inline-flex items-center px-2.5 py-0.5 rounded dark:bg-indigo-200 dark:text-indigo-800">
                ...
                @foreach($articles as $article)
                    <article
                        ...
                        <div class="flex justify-center mb-3">
                            ...
                            @else
                                <img class="h-auto rounded-lg" src="{{asset('default.png')}}" alt="article image">
                            @endif
                        </div>
                      ...
                      {{$article->category->name}}
                        ...
                        <div class="flex justify-between items-center">
                            <div class="flex items-center space-x-2">
                                <b>Author by:</b><span
                                    class="font-medium dark:text-white">{{$article->author->name}}</span>
                            </div>
