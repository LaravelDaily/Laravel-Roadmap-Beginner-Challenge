        ...
        <section class="mt-10 overflow-x-scroll">
            <form method="post" action="{{ route('dashboard.articles.update', $article->id) }}" enctype="multipart/form-data">
                ...
                <section>
                    ...
                    <x-text-input id="title" name="title" type="text" class="mt-1 block w-full" autofocus :value="old('title', $article->title)" />
                    <x-input-error :messages="$errors->get('title')" class="mt-2" />
                </section>
                <section class="mt-2">
                    <x-input-label for="full_text" class="text-lg" value="full text" />
                    <x-textarea id="full_text" name="full_text" class="mt-1 block w-full" rows="5">{{ old('full_text', $article->full_text) }}</x-textarea>
                ...
                <section class="mt-2">
                    ...
                    <img class="w-20 h-20 rounded mt-5" src="{{ asset($article->image) }}"/>
                </section>
                    ...
                    <x-select id="categories" name="categories[]" type="text" class="mt-1 block w-full" multiple>
                        @foreach($categories as $category)
                            <option
                                value="{{ $category->id }}"
                                @selected(
                                    in_array($category->id, old('categories', $article->categories->pluck('id')->toArray()) ?? [])
                                )
                            >
                                {{ $category->name }}
                            </option>
                ...
                <x-primary-button class="mt-10 !bg-green-300">edit</x-primary-button>
            </form>
        </section>
    </section>
</x-app-layout>
